      ...
      <div style={styles.grid}>
        <div style={styles.card}>
          ...
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', margin: '0 auto' }}>
            {[20, 40, 60, 80, 100].map(v => { const pts = dims.map((d) => getPoint(dims.indexOf(d), v)); return <polygon key={`g${v}`} points={pts.map(p => `${p.x},${p.y}`).join(' ')} fill="none" stroke="var(--border)" />; })}
            {dims.map((d) => { const end = getPoint(dims.indexOf(d), 100); return <line key={`ax-${d}`} x1={cx} y1={cy} x2={end.x} y2={end.y} stroke="var(--border)" />; })}
            <polygon points={polygon} fill="var(--accent-subtle)" stroke="var(--color-accent)" strokeWidth="2" />
...
const styles = {
  ...
  card: { padding: '1.5rem', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)' },
  ...
  twinCard: { padding: '1.5rem', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)', marginBottom: '1.25rem' },
  twinGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' },
  twinItem: { display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem', background: 'var(--bg-elevated)', borderRadius: '10px' },
