...
export default function AuthPage() {
  ...
  useEffect(() => {
    let done = 0;
    const check = () => { done++; if (done >= 2) setHydrated(true); };
    const unsub1 = useAuthStore.persist.onFinishHydration(() => check());
    const unsub2 = useFounderStore.persist.onFinishHydration(() => check());
    if (useAuthStore.persist.hasHydrated()) check();
    if (useFounderStore.persist.hasHydrated()) check();
    return () => { unsub1(); unsub2(); };
  }, []);

  useEffect(() => {
    if (!hydrated) return;
    if (token && useFounderStore.getState().profile) navigate('/dashboard', { replace: true });
          ...
          {token ? (
            <>
              ...
              <button className="btn btn-primary btn-lg" style={{ width: '100%' }} onClick={() => {
                const p = useFounderStore.getState().profile;
                navigate(p ? '/dashboard' : '/onboarding');
              }}>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textAlign: 'left' }}>
                ...
                <div>
                  ...
                  {mode === 'login' && (
                    <button type="button" style={{ background: 'none', border: 'none', color: 'var(--accent-dark)', cursor: 'pointer', fontSize: '0.8125rem', fontWeight: 500, padding: '0.25rem 0', textAlign: 'right', width: '100%' }} onClick={() => navigate('/reset-password')}>
                      Forgot password?
                    </button>
                  )}
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginTop: '1.5rem' }}>
                ...
                <button
                  type="button"
                  style={{ background: 'none', border: 'none', color: 'var(--accent-dark)', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '0.25rem' }}
