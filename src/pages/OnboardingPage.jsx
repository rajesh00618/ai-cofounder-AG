  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--bg)' }}>
      <div style={{ width: '100%', maxWidth: '600px' }}>
        ...
        <ThreeDCard intensity={3} key={customMode ? 'custom' : onboardingStep}>
          <SlashCard variant="accent" style={{ textAlign: 'center' }}>
            <div style={{
            ...
            {customMode ? (
              ...
              <div className="stagger" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {currentQ.options.map((opt) => (
                  <button key={`opt-${opt.slice(0, 15)}`} onClick={() => handleSelect(opt)} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '1rem 1.5rem',
                    background: onboardingAnswers[onboardingStep + 1] === opt ? 'var(--accent-subtle)' : 'rgba(255,255,255,0.02)',
                    border: `1px solid ${onboardingAnswers[onboardingStep + 1] === opt ? 'rgba(99,102,241,0.25)' : 'var(--border)'}`,
                    ...
                    <span>{opt}</span>
                    {onboardingAnswers[onboardingStep + 1] === opt && (
                      <CheckCircle2 size={18} style={{ color: 'var(--accent-light)', animation: 'bounceIn 0.4s ease-out' }} />
                    )}
                  </button>
                ))}
                <button onClick={() => setCustomMode(true)} style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                  padding: '1rem 1.5rem', marginTop: '0.25rem',
          ...
          {ONBOARDING_QUESTIONS.map((q) => (
            <div key={`dot-${q.id}`} style={{
              ...
              background: (q.id - 1) < onboardingStep ? 'var(--accent)' :
                (q.id - 1) === onboardingStep ? 'var(--accent-light)' : 'rgba(255,255,255,0.06)',
