...
function ConfidenceMeter({ value, reason }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', marginTop: '0.75rem' }}>
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', margin: '0 auto' }}>
      {[20,40,60,80,100].map(v => {
        ...
        return <polygon key={`sg-${v}`} points={pts.map(p=>`${p.x},${p.y}`).join(' ')} fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth="1" />;
      })}
      {dims.map((d) => {
        const end = getPoint(dims.indexOf(d), 100);
        return <line key={`sl-${d}`} x1={cx} y1={cy} x2={end.x} y2={end.y} stroke="rgba(255,255,255,0.04)" strokeWidth="1" />;
      })}
      <polygon points={polygon} fill="rgba(99,102,241,0.1)" stroke="var(--accent)" strokeWidth="2" />
      {dims.map((d) => {
...
export default function GoalPage() {
  ...
  const handleProceed = () => { navigate('/business-planning'); };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--bg)' }}>
      <div style={{ width: '100%', maxWidth: '700px' }}>
          ...
          <ThreeDCard intensity={4}>
            <SlashCard variant="accent" style={{ textAlign: 'center' }}>
              <div style={{
                ...
                color: 'white', margin: '0 auto 1.5rem', animation: 'float 3s ease-in-out infinite',
              }}>
                <Sparkles size={28} />
              </div>
              ...
              <p style={{ color: 'var(--accent-light)', fontWeight: 500, marginBottom: '1.5rem' }}>Tell me your biggest goal today.</p>
          ...
          <ThreeDCard intensity={3}>
            <SlashCard variant="accent" style={{ textAlign: 'center' }}>
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: 'var(--accent-light)', margin: '0 auto 1.25rem',
              }}>
                <Target size={24} />
              </div>
        ...
        {phase === PHASES.CLARIFYING && !thinking && clarQuestions.length > 0 && (
          <ThreeDCard intensity={3} key={clarCustomMode ? 'clar-custom' : clarStep}>
              ...
              {clarCustomMode ? (
                ...
                <div className="stagger" style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  ...
                  <button onClick={() => setClarCustomMode(true)} style={{
                    ...
                    color: 'var(--accent-light)', background: 'transparent',
                    marginTop: '0.25rem',
                  }}>
        ...
        {phase === PHASES.REALITY && reality && (
          <ThreeDCard intensity={4}>
            <SlashCard variant="accent">
              ...
              <div style={{ padding: '0.75rem 1rem', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius)', marginBottom: '1.5rem', fontSize: '0.9375rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '2rem', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius)', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', padding: '1rem', background: 'var(--accent-subtle)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 'var(--radius)', marginTop: '1rem', fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                <Sparkles size={16} style={{ color: 'var(--accent-light)', marginTop: 2 }} />
            ...
            <SlashCard variant="accent">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <button type="button" onClick={() => handleSelectAlternative(negotiation.current)} style={{
                  padding: '1.25rem', background: selectedAlt === negotiation.current ? 'var(--accent-subtle)' : 'rgba(255,255,255,0.02)',
                  border: `1px solid ${selectedAlt === negotiation.current ? 'rgba(99,102,241,0.3)' : 'var(--border)'}`,
                ...
                {(negotiation.alternatives || []).map((alt, i) => (
                  <button key={`alt-${alt.label?.slice(0, 15) || i}`} type="button" onClick={() => handleSelectAlternative(alt)} style={{
                    padding: '1.25rem', background: selectedAlt === alt ? 'var(--accent-subtle)' : 'rgba(255,255,255,0.02)',
                    border: `1px solid ${selectedAlt === alt ? 'rgba(99,102,241,0.3)' : 'var(--border)'}`,
                    borderRadius: 'var(--radius)', cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
