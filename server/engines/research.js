...
export const getResearch = async (apiKey, businessContext) => {
  ...
  try {
    const queries = [];
    if (businessContext?.blueprint?.problem) queries.push(`${businessContext.blueprint.problem.slice(0, 80)} market size 2025`);
    if (businessContext?.blueprint?.industry) queries.push(`${businessContext.blueprint.industry} market trends 2025`);
    if (businessContext?.blueprint?.competitors?.length) queries.push(`${businessContext.blueprint.competitors[0]} competitor analysis 2025`);
    queries.push('latest startup funding trends 2025');

    const webData = await searchWebBatch(queries.slice(0, 3));
    searchResults = Object.values(webData).flat().slice(0, 10);
    console.log(`[Research] Got ${searchResults.length} real web results`);
  } catch (err) {
    console.warn(`[Research] Web search failed: ${err.message}`);
  }
...
export const getOpportunities = async (apiKey, businessContext) => {
  ...
  try {
    const queries = [];
    if (businessContext?.blueprint?.industry) queries.push(`${businessContext.blueprint.industry} startup grants 2025`);
    queries.push('startup accelerators accepting applications 2025');
    queries.push('small business grants 2025');

    const webData = await searchWebBatch(queries.slice(0, 2));
    searchResults = Object.values(webData).flat().slice(0, 8);
  } catch (err) {
    console.warn(`[Research] Opportunity search failed: ${err.message}`);
  }
...
export const getMorningBriefing = async (apiKey, profile, businessContext) => {
  ...
  try {
    ...
    console.warn(`[Research] Briefing search failed: ${err.message}`);
  }
